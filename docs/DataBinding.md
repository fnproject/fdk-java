...
The SDK supports binding JSON data types to POJOs. This is implemented using the Jackson API.
...
To get the most flexibility in handling data in and out of your function, the SDK also provides an abstraction of the raw fn Java FDK events received or returned by the function by means of the [InputEvent](../api/src/main/java/com.fnproject.fn/api/InputEvent.java) and [OutputEvent](../api/src/main/java/com.fnproject.fn/api/OutputEvent.java) interfaces.

Using this approach allows you to:

- access the headers of the incoming request;
- read the request body as an `InputStream`;
- control headers or response status of the response.
...
Below is aa function that looks at both the body of the request and a custom `X-My-Header` header.
...
After you have [deployed the function](../README.md)  you can call it via HTTP as follows: 
...
To help with this, you can extend the functionality of the SDK by providing your own functionality to convert between data types. See the [Extending Data Binding](ExtendingDataBinding.md) tutorial for details of how to do this.
